...
void
build_events(int **ranges, int *n, int threshold, int gap) {
    //create 2 tupes with + /-180u s
    // Then merge with single For loop
    ourrangeindex = 0;
    for (i = 0; i < sum_waveform_n; ++i) {
        if (sum_waveform[i] > threshold) {
            //Above threshold
            int		start = i - gap;
            int		stop = i + gap;
