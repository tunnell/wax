...
u_int32_t ProcessTimeRangeTask(int64_t t0, int64_t t1,
    ...
    for (int i = 0; i < occurence_mapping_to_trigger_ranges.size(); ++i) {
	    ...
	    // seem to know about int64.  Therefore, we have to cast.
            ...
            bab.append((long long int) trigger_event_ranges[2*builder_mapping] * reduction_factor);
            bab.append((long long int) trigger_event_ranges[2*builder_mapping + 1] * reduction_factor);
