...
class Test_save_time_range(unittest.TestCase):
    ...
    def test_single_peak_int(self):
        x = EventBuilding.get_index_mask_for_trigger(10, 5, self.range_around_trigger).tolist()
    ...
    def test_multi_peak_list_no_overlap(self):
        x = EventBuilding.get_index_mask_for_trigger(10, [2, 9], self.range_around_trigger).tolist()
        self.assertEqual([True, True, True, True, True, False, True, True, True, True],
                         x)
    def test_multi_peak_list_overlap(self):
        x = EventBuilding.get_index_mask_for_trigger(10, [5, 15], self.range_around_trigger).tolist()
        self.assertEqual(self.single,
                         x)

    def test_overrun(self):
        x = EventBuilding.get_index_mask_for_trigger(10, [0], self.range_around_trigger).tolist()
        self.assertEqual([True, True, True, False, False, False, False, False, False, False],
                         x)

    def test_single_peak_float_exception(self):
         with self.assertRaises(ValueError):
            EventBuilding.get_index_mask_for_trigger(10, 5.0, self.range_around_trigger)

    def test_size_not_int(self):
         with self.assertRaises(ValueError):
            EventBuilding.get_index_mask_for_trigger([10], 5, self.range_around_trigger)
